...
public class Step1 {
  ...
  public Step1(final GRBEnv env, final int nbRounds, final int nExtbRounds, final int nExtfRounds, final int regime, final int blocksize) throws GRBException {
    ...
    DTKupper = (regime == 0) ? null : new Step1Tweakey(model, nbRounds+1, regime);
    DXlower = new GRBVar[nbRounds+1][4][4];
    freeXlower = new GRBVar[nbRounds][4][4];
    freeSBlower = new GRBVar[nbRounds][4][4];
    DTKlower = (regime == 0) ? null : new Step1Tweakey(model, nbRounds+1, regime);
  ...
  private void articleTrails() throws GRBException {
    ...
    // SKINNY-128-256
      ...
      model.addConstr(freeXupper[9][0][1], GRB.EQUAL, 0.0, "debug");
      model.addConstr(freeXupper[10][0][1], GRB.EQUAL, 0.0, "debug");
      model.addConstr(freeXupper[10][1][1], GRB.EQUAL, 0.0, "debug");
      model.addConstr(freeXupper[10][2][3], GRB.EQUAL, 0.0, "debug");
      model.addConstr(freeXupper[10][3][1], GRB.EQUAL, 0.0, "debug");
      model.addConstr(freeXupper[11][0][0], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXupper[11][2][0], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXupper[13][1][3], GRB.EQUAL, 1.0, "debug");
      for (int round = 14; round < 19; round++)
        for (int i = 0; i < 4; i++)
          for (int j = 0; j < 4; j++) {
            if (round == 14 && i == 0 && j == 1)
              model.addConstr(DXlower[round][i][j], GRB.EQUAL, 1.0, "debug");
      ...
      model.addConstr(freeXlower[9][1][0], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[9][1][2], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[9][2][1], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[10][0][3], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[10][2][2], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[11][0][3], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[11][2][1], GRB.EQUAL, 0.0, "debug");
    }
    // SKINNY-128-256
      ...
      model.addConstr(freeXupper[7][0][1], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXupper[7][1][1], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXupper[7][3][1], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXupper[8][1][0], GRB.EQUAL, 1.0, "debug");
      for (int round = 10; round < 18; round++)
        for (int i = 0; i < 4; i++)
          for (int j = 0; j < 4; j++) {
            if (round == 10 && i == 0 && j == 1 ||
                round == 10 && i == 0 && j == 3 ||
    ...
    // SKINNY-64-192
      ...
      for (int round = 15; round < 22; round++)
        for (int i = 0; i < 4; i++)
          for (int j = 0; j < 4; j++) {
            if (round == 15 && i == 0 && j == 1)
              model.addConstr(DXlower[round][i][j], GRB.EQUAL, 1.0, "debug");
      ...
      for (int round = 11; round < 13; round++)
        for (int i = 0; i < 4; i++)
          for (int j = 0; j < 4; j++) {
            if (round == 11 && i == 0 && j == 2 ||
                round == 12 && i == 0 && j == 2 ||
                round == 12 && i == 1 && j == 2 ||
                round == 12 && i == 3 && j == 2)
              model.addConstr(freeXupper[round][i][j], GRB.EQUAL, 1.0, "debug");
            else
              model.addConstr(freeXupper[round][i][j], GRB.EQUAL, 0.0, "debug");
          }
       for (int round = 12; round < 14; round++)
        for (int i = 0; i < 4; i++)
          for (int j = 0; j < 4; j++) {
            if (round == 12 && i == 0 && j == 3 ||
                round == 12 && i == 1 && j == 0 ||
                ...
                round == 12 && i == 3 && j == 0 ||
                round == 13 && i == 2 && j == 0)
              model.addConstr(freeXlower[round][i][j], GRB.EQUAL, 1.0, "debug");
            else
              model.addConstr(freeXlower[round][i][j], GRB.EQUAL, 0.0, "debug");
          }
    }
    // SKINNY-128-384
      ...
      model.addConstr(freeXupper[12][0][1], GRB.EQUAL, 0.0, "debug");
      model.addConstr(freeXupper[12][2][3], GRB.EQUAL, 0.0, "debug");
      model.addConstr(freeXupper[15][1][3], GRB.EQUAL, 1.0, "debug");
      for (int round = 16; round < 23; round++)
        for (int i = 0; i < 4; i++)
          for (int j = 0; j < 4; j++) {
            if (round == 16 && i == 0 && j == 1)
              model.addConstr(DXlower[round][i][j], GRB.EQUAL, 1.0, "debug");
      ...
      model.addConstr(freeXlower[11][0][3], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[11][1][0], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[11][1][2], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[11][2][1], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[12][0][3], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[12][2][2], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[13][0][1], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[13][0][3], GRB.EQUAL, 1.0, "debug");
      model.addConstr(freeXlower[13][2][3], GRB.EQUAL, 1.0, "debug");
    }
